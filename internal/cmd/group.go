...
var groupsListCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		printTable([]string{"Name", "Version", "Locations", "Sleeping"}, groupsTable(groups))
		return nil
	},
}
...
func groupsTable(groups []turso.Group) [][]string {
	...
	for _, group := range groups {
		row := []string{group.Name, group.Version, formatLocations(group.Locations, group.Primary), formatBool(group.Archived)}
